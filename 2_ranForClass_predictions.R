...
ranForClass_grid_search.1 <- function(df_class, df_feat, ID_obj, lambdas){
  library(glmnet)
  ...
  for( i in 1:length(ID_obj[[1]]) ){
    ...
    for( j in 1:length(lambdas) ){
      if(j == 0){
        ...
        model <- glmnet(x, y.glmnet, alpha = 1, lambda = lambda)
      ...
      fit <- predict(object = RF_model, newdata = newx, type="prob")
      df_pred <- data.frame( actual = df_class.test[,1], pred = fit[,2], row.names = row.names(df_class.test) )
...
RF_grid_predictions_weights <- ranForClass_grid_search.1(df_class = m.class, df_feat = m.feat.class, ID_obj = ID_obj.trainTest, lambdas = lambdas)
RF_grid_predictions <- RF_grid_predictions_weights[[1]]
RF_grid_weights <- RF_grid_predictions_weights[[2]]
...
saveRDS( object = RF_grid_predictions, file = outNm.predictions )
saveRDS( object = RF_grid_weights, file = outNm.weights )
