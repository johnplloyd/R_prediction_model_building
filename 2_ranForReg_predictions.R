...
ranForReg_grid_search.1 <- function(df_class, df_feat, ID_obj, lambdas){
  library(glmnet)
  ...
  for( i in 1:length(ID_obj[[1]]) ){
    ...
    for( j in 1:length(lambdas) ){
      if(j == 0){
        ...
        x <- as.matrix(df_feat.train.sel)
        newx <- as.matrix(df_feat.test.sel)
      }
      
      RF_model <- randomForest(x = x, y = y, ntree = 500)
      fit <- predict(object = RF_model, newdata = newx)
...
lambdas <- c( 1e-3, 5e-3, 1e-2, 5e-2, 1e-1, 5e-1 )

RF_grid_predictions_weights <- ranForReg_grid_search.1(df_class = m.class, df_feat = m.feat.class, ID_obj = ID_obj.trainTest, lambdas = lambdas)
